...
- 외부 함수는 오리지널 함수를 인자로 받아서 주소를 기억하고, wrapper 함수 자체를 리턴해주는 껍데기
- @가 붙는 순간 파이썬은 내부적으로 기존 함수 이름을 wrapper()로 바꿔치기 해서 겉보기엔 오리지널 함수를 호출하는 것 처럼 보여도, 사실은 wrapper()를 호출하는 것이다.
- wrapper는 오리지널 함수가 어떤 인자를 받을지 모르니까 일단 무한대 그릇인 *args, **kwargs 가변 변수로 받아놓고, 오리지널 함수에게 패스해준다. 
