...
앞뒤, 중간 공백을 다 제거한 깔끔한 리스트를 만들어준다. 구분자는 엑셀에서 문자열 나누기 할 떄와 같다. 
str.split() 인자를 주지 않으면 모든 공백, 탭, 줄바꿈을 자동으로 처리해준다. 
"_".join(list)는 요소들 사이에 언더바를 넣어 문자열로 합쳐준다. 
"_".join(list) 여기서 list 요소는 int일 수 없다. str이어야 한다. int라면 str()으로 변경해서 사용한다. 
    list_of_numbers = [1, 2, 3] # str() 으로 바꾸지 않으면 에러남 
    print("_".join(str(num) for num in list_of_numbers))
...
f-string format 
변수명:[채울 문자][정렬 방향][전체 칸수][데이터 타입]
{minutes}:02d  해석-> 0으로 채워라 2칸, decimal 정수타입으로 

1. 천 단위 콤마  :,
print(f"결제 금액: {price:,}원")  

2. 소수점 자리 제한  :.2f 
print(f"정확도 {accuracy:.2f}%") -> .2 소수점 아래 두자리 float 

3. 천 단위 + 소수점 :,.2f
print(f"매출 £{price:,.2f}") -> £8,123,232.98
=======================================================================
