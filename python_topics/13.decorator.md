...
``` @performance_tracker를 쓰면 파이썬 내부에서 일어나는 일
my_func = performance_tracker(my_func)
```
1. performance_tracker(my_func)를 실행하면, 내부의 wrapper 함수가 통째로 리턴돼.
2. 그리고 기존의 my_func라는 이름은 이제 원본 함수가 아니라, 리턴된 wrapper 함수를 가리키게 돼. 이름이 바꿔치기 된 거지!
