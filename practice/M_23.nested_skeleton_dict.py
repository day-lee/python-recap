...
def process_orders(orders: list, target_categories: set) -> dict:
    ...
    """skeleton dict 모범 답안
    # =======================================================================
    # # 1. 초기 뼈대인 스켈레톤 딕셔너리를 먼저 만들어둔다. (만약 구조가 정해져 있다면 내부 키도 먼저 만들어 둘 수 있음)
    # result = {}

    # 2. collections.defaultdict() 사용해서 중첩 구조를 설계해둠
    # from collections import defaultdict 
    # result = defaultdict(lambda: {"buyers": set(), "total_revenue":0})

    # 3. guard clauses 전략: 조건이 안맞으면 먼저 쳐내고 탈출하기 
    # if order["category"] not in target_cateogries: continue 

    # =======================================================================
    
        # 동적으로 미리 뼈대 생성 
    result = {
        category: {"buyers": set(), "total_revenue":0}
    ...
    for order in orders:
        try: 
            ...
            # 이미 뼈대 만들어 둔 것에 업데이트 - if 로 검사하고 진행할 필요 없음 
    ...
    #     buyer = order["buyer"]
    #     try:
    #         if order["price"] and order["quantity"] and isinstance(order["price"], (int, float)) and isinstance( order["quantity"], (int, float)):
    #             print('no order')
    #             if category in target_categories:
    #                 if category not in result:
    #                     result[category] = { "buyers": set(), "total_revenue": 0}
    #                 result[category]["buyers"].add(buyer)
    #                 result[category]["total_revenue"] +=  order["quantity"] * order["price"] 
