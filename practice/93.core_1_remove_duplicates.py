...
def problem_01_remove_duplicates_1(items):
    print(list(set(items)))



    # 2-1. membership check 특정 값이 자료 구조 안에 존재하는지 확인하기 O(N2) 
def problem_01_remove_duplicates_2_1(items):
...
def problem_01_remove_duplicates_2_2(items):
    seen = set() 
    result = []
    for item in items:
        if item not in seen:
    ...
    #3. dict.fromkeys(iterable, value) : key는 중복 허용 x, value는 None이 기본값
def problem_01_remove_duplicates_3(items):
...
problem_01_remove_duplicates_1([3, 1, 2, 3, 2, 4, 1]) 
problem_01_remove_duplicates_2_1([3, 1, 2, 3, 2, 4, 1]) 
problem_01_remove_duplicates_2_2([3, 1, 2, 3, 2, 4, 1]) 
problem_01_remove_duplicates_3([3, 1, 2, 3, 2, 4, 1]) 
