...
def clean_user_profile(**kwargs):
    ...
    # city = kwargs.get('city', '').strip().lower() 
    # kwargs['name'] = name
    # kwargs['email'] = email
    # kwargs['city'] = city 
    # return kwargs
    ...
    #         kwargs[key] = value.lower().strip() 
    # return kwargs

    # return { key: value.lower().strip() if isinstance(value, str) else value for key, value in kwargs.items() }
