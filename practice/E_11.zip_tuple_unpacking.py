"""
Address Book Name-to-Phone Mapping
Combine two separate lists, names and phone_numbers, 
into a single dictionary where names are keys and phone numbers are values.
However, you must filter out invalid data. 
Only include pairs where the phone number is properly formatted with hyphens (-).
...
def clean_address_book(names, phones):
    ...
    """
    result = {name: phone for name, phone in zip(names, phones) if '-' in phone}
    return result
