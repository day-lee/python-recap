...
def clean_and_tag_reviews(*args, ratings=None, **kwargs):
    ...
    # print(args) # user list
    # print(ratings) # ratings list [4.5, '최고예요', 1.2, 5.0, -0.5] 
    # print(kwargs) # user: review - strip, Upper, in GOOD or BEST
    # if ratings is None:
    #     return {}
    # result = {}
    # for user, rating in zip(args, ratings):
    #     try: 
    #         if not isinstance(rating, (int, float)):
    #             raise ValueError(f"[_에러_] {user}: 올바르지 않은 평점 - type")
    #         if rating < 0 or rating > 5:
    #             raise ValueError(f"[_에러_] {user}: 올바르지 않은 평점 - range")
    #     except ValueError as e:
    #         print(e)
    #         continue # 이게 빠지면 안됨. 에러만 발생시켰지 아래 코드로 계속 내려감. 이번 이터레이션 건너뛰고, 다음 유저로 넘어가라고 명령 해줘야함.  

    #     # 레이팅 필터된 유저의 리뷰 텍스트를 가져와서 정제하고 필터링. 
    #     review = kwargs.get(user, "").strip().upper()
    #     if "GOOD" in review or "BEST" in review:
    #         result[user] = rating
    # return result
    ...
    # for id, rating in zip(args, ratings):
    #     try:
    #         review = kwargs.get(id, 'No Review')
    #         if not isinstance(rating, (int, float)) or rating < 1 or rating > 5:
    #             raise ValueError(f"[_에러_] {id}: 올바르지 않은 평점")
    #             continue 
    #         review = review.strip().upper()
    #         if "GOOD" in review or "BEST" in review:
    #             result[id] = rating 
    ...
    # filtered_user = {}
    # result = {}
    # for name, score in zip(args, ratings):
    #     try:
    #         if not isinstance(score, (int, float)) or score < 0:
    #             raise ValueError
    #         filtered_user[name] = score
    #     except ValueError:
    #         print(f"[Error] {name}: rating is wrong format")
    # for user, review in kwargs.items():
    #     if not user:
    #         print("no user found")
    #     if user in filtered_user:
    #         cleaned_review = review.strip().lower()
    #         if "best" in cleaned_review or "good" in cleaned_review:
    #             result[user] = filtered_user.get(user)
    # return result
