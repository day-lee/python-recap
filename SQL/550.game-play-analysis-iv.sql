...
- lead(기준 컬럼)로 다음에 오는 로우 구함(정렬해서 날짜순 다음에 오는 로우, 날짜순으로 바로 다음날이 아닐수도 있음) 
- 랭크로 1인 것만 필터링 해놓고, 첫 로그인과 다음 로그인의 날짜 차이가 DATEDIFF(미래, 과거) = 1 인 것만 조건부로 카운팅함. 인자 순서 주의 

- LEAD() 다음 로우가 없다면 null을 반환한다. 
